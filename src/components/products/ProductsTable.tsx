...
export function ProductsTable({
  products, isLoading, isError, onRetry, onEdit, onEditDraft, onDelete, onCopy, onDeleteDraft,
}: {
  products: (AdminProduct | ProductDraft)[];
  isLoading: boolean;
  isError: boolean;
  onRetry: () => void;
      ...
      <Table>
        ...
        <TableBody>
          {products.map((p, rowIdx) => {
            ...
            const displayId = isDraft ? p.draftId : p.id;
            const displayName = isDraft ? p.nameEn : p.name;
            const displayDescription = isDraft ? p.descriptionEn : ("description" in p ? p.description : "");
            const displayCategory = isDraft ? "" : ("categoryName" in p ? p.categoryName : "");
            const displayPrice = p.price;
            const displayDiscountedPrice = isDraft ? p.price : ("discountedPrice" in p ? p.discountedPrice : p.price);
            const displayImageUrl = p.imageUrl;
            const displayIsAvailable = isDraft ? false : ("isAvailable" in p ? p.isAvailable : false);

            return (
              <TableRow
                key={isDraft ? p.draftId : p.id}
                className={cn(
              ...
              >
                <TableCell className="py-2.5 pl-4 pr-2">
                  <ProductImage url={displayImageUrl} alt={displayName} />
                </TableCell>
                <TableCell className="py-2.5">
                  <span className="font-mono text-sm font-medium text-primary">
                    {isDraft ? displayId : `#${displayId}`}
                  </span>
                </TableCell>
                <TableCell className="py-2.5">
                  <div className="flex items-center gap-2">
                    <div className="flex-1">
                      ...
                      {displayDescription && (
                        <p className="text-xs text-muted-foreground line-clamp-1 max-w-xs">{displayDescription}</p>
                      )}
                    </div>
                    {isDraft && (
                      <Badge className="bg-amber-500/15 text-amber-700 dark:text-amber-300 border border-amber-500/25 hover:bg-amber-500/20 h-5 px-2 text-[10px] font-mono uppercase tracking-wide">
                        DRAFT
                      </Badge>
                    )}
                  </div>
                ...
                <TableCell className="py-2.5 text-right font-mono text-sm tabular-nums">
                  {!isDraft && displayDiscountedPrice < displayPrice ? (
                    <span className="font-semibold text-primary">{fmt(displayDiscountedPrice)}</span>
                  ) : (
                    <span className="text-muted-foreground/60">—</span>
                  )}
                </TableCell>
                <TableCell className="py-2.5">
                  {!isDraft && displayIsAvailable ? (
                  ...
                  <div className="flex items-center justify-end gap-1">
                    {!isDraft && (
                      <Button
                        variant="ghost"
                        size="icon"
                        onClick={() => onCopy(p as AdminProduct)}
                    ...
                    <Button
                      variant="ghost"
                      size="icon"
                      onClick={() => isDraft ? onEditDraft(p.draftId) : onEdit(p as AdminProduct)}
                      className="h-8 w-8 text-muted-foreground hover:text-primary hover:bg-primary/10"
                      title="Edit"
                    >
                    ...
                    <Button
                      ...
                      onClick={() => {
                        if (isDraft) {
                          if (confirm(`Delete draft "${displayName}"?`)) {
                            onDeleteDraft(p.draftId);
                          }
                        } else {
                          onDelete(p as AdminProduct);
                        }
                      }}
